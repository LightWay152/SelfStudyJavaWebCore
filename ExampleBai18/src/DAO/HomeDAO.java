...
public class HomeDAO {
	...
	public static void UploadSingleFile(HttpServletRequest request,HttpServletResponse response) throws ServletException, IOException{
		final String Address = "E:\\WorkSpace\\JavaWebJSPServletCore\\BackupDatabase\\UploadSingleFile";
		...
		try {
			...
			while (iter.hasNext()) {
			    ...
			    if (!item.isFormField()) {
			    	...
			    	try {
			    		if(check==true){
							request.setAttribute("message", "File existed. Required: test file again!");
			    		}else{
